/*
  ...
  Гамбургер может быть с одной из нескольких видов начинок topping (обязательно):
...
class Hamburger {
    ...
    //  */
    constructor(size, stuffing) {    //сюда заходит Маленький гамбургер с начинкой из сыра
      this._size = size;         // изначально здесь гамб маленького размера
      this._stuffing = stuffing;  // здесь начинка из сыра
      this._toppings = [];
    }

    /**
     * Добавить topping к гамбургеру. Можно добавить несколько topping,
     *  при условии, что они разные.
     //@param {String} topping - Тип добавки
     */

    addTopping(topping) {   // Добавка из приправы с ценой 10  и калориями 0
       if (!this._toppings.includes(topping)){  /// добавь в масив если там нету
    ...
    removeTopping(topping) {
      ...
      if (this._toppings.includes(topping)){
        let topInd = this._toppings.indexOf(topping);
        // console.log(topInd);
        this._toppings.splice(0,1);
          }
    }
    /**
     ...
     *  Hamburger.TOPPING_*
     * Попробуйте сделать это геттером чтобы можно было обращаться
     *  как obj.toppings и нам вернет массив добавок
     */
    get toppings() {
     ...
     * Попробуйте сделать это геттером чтобы можно было обращаться как obj.stuffing
     *  и нам вернет начинку
     */
    get stuffing() {
     ...
     *
     * Попробуйте сделать это геттером чтобы можно было обращаться 
     * //как obj.price и нам вернет сумму.
     */
    calculatePrice() {
      let counter = 0;
      for (let el of this._toppings){
        counter += Hamburger.TOPPINGS[el].price;
      }
      let priceSize = Hamburger.SIZES[this._size].price+Hamburger.STUFFINGS[this._stuffing].price+counter;
     ...
     *
     * Попробуйте сделать это геттером чтобы можно было обращаться
     * // как obj.calories и нам вернет сумму.
     */
    calculateCalories() {
      let sum = 0;
      for (let el of this._toppings){
        sum += Hamburger.TOPPINGS[el].calories;
      }
      let sumCalories = Hamburger.SIZES[this._size].calories+Hamburger.STUFFINGS[this._stuffing].calories
      +sum;
  ...
  */
  ...
  Hamburger.STUFFINGS = {
    ...
    [Hamburger.STUFFING_CHEESE]: {   /// это все равно что 'STUFFING_CHEESE'/ это название ключа  
      price: 15,
      calories: 20,
    },
  ...
  // Добавка из приправы с ценой 10  и калориями 0
  hamburger.addTopping(Hamburger.TOPPING_SPICE);
  console.log(hamburger);

            // hamburger.getStuffing('STUFFING_CHEESE');
            // console.log(hamburger);
  ...
  console.log("Price with sauce: ", hamburger.calculatePrice(Hamburger.SIZE_SMALL ));
